...
# -------------------------
# Phase 5B: sensible defaults provision
# -------------------------
def provision_company_defaults(company: Company) -> None:
    """Idempotently create core defaults for a new company.

    Goals:
    - Ensure numbering scheme exists
    - Ensure minimal chart of accounts exists
    - Ensure a default document template exists for each doc_type
    """
    from documents.models import DocumentTemplate, DocumentType
    from documents.services import ensure_numbering_scheme
    from accounting.models import ensure_default_chart

    ensure_numbering_scheme(company)
    ensure_default_chart(company)

    for dt in (DocumentType.INVOICE, DocumentType.ESTIMATE, DocumentType.PROPOSAL):
