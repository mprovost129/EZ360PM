...
def public_invoice_pay(request, token):
    ...
    from payments.models import StripeConnectAccount
    ...
    # If company has Stripe Connect payouts configured, route funds to the connected account.
    destination_acct = ""
    try:
        sca = StripeConnectAccount.objects.filter(company_id=inv.company_id, deleted_at__isnull=True).first()
        if sca and sca.is_ready and sca.stripe_account_id:
            destination_acct = (sca.stripe_account_id or "").strip()
    except Exception:
        destination_acct = ""
    ...
    if destination_acct:
        payment_intent_data["transfer_data"] = {"destination": destination_acct}
...
def document_edit(request, doc_type: str, pk):
    ...
    if request.method == "POST":
        ...
        if request.POST.get("action") == "send_email":
            can_edit_number = (
                (not is_locked)
                and (doc.status == DocumentStatus.DRAFT)
                and (employee.role in {EmployeeRole.MANAGER, EmployeeRole.ADMIN, EmployeeRole.OWNER})
            )
            form = DocumentForm(request.POST, instance=doc, company=company, doc_type=doc_type, can_edit_number=can_edit_number)
            ...
            if form.is_valid() and formset.is_valid():
                with transaction.atomic():
                    ...
                    if not doc.number:
                        ...
                        doc.number = allocate_document_number(
                            company,
                            doc_type,
                            project=doc.project,
                            use_project_numbering=bool(getattr(doc, "use_project_numbering", False)),
                        )
        ...
        if form.is_valid() and formset.is_valid():
            ...
            # After save: allow "save + open" actions so Print/PDF always reflect current form state.
            if action == "save_and_pdf":
                return redirect(reverse(f"documents:{doc_type}_pdf", kwargs={"pk": doc.pk}))
            if action == "save_and_print":
                return redirect(reverse(f"documents:{doc_type}_print", kwargs={"pk": doc.pk}))
        ...
        form = DocumentForm(instance=doc, company=company, doc_type=doc_type, can_edit_number=can_edit_number)
        formset = DocumentLineItemFormSet(
            instance=doc,
            form_kwargs={
                'company_default_taxable': bool(getattr(company, 'default_line_items_taxable', False)),
