...
- Added **ops notification emails** (separate from alerts) in Ops SiteConfig:
  ...
  - toggles for **new company signup** and **subscription becomes active**
- Company onboarding now triggers an ops signup email (post-commit).
- Stripe subscription sync sends a one-time “became ACTIVE” ops email and records `CompanySubscription.ops_notified_active_at` to prevent duplicates.
...
## 2026-02-19 — DB-backed Pricing Catalog (DONE)

- Added **billing.PlanCatalog** (Starter/Professional/Premium) to store:
  - Monthly + annual display prices
  - Included seats + trial days
  - Optional Stripe **base plan** Price IDs (monthly/annual)
- Added **billing.SeatAddonConfig** singleton to store:
  - Extra seat monthly/annual display prices
  - Optional Stripe **seat add-on** Price IDs
- Updated **Pricing page** to render prices from the DB catalog (no hard-coded pricing).
- Updated **Stripe Checkout** subscription creation to prefer DB Price IDs (fallback to settings lookup map).
- Trial days now prefer per-plan catalog trial days, then Ops SiteConfig, then settings fallback.
- Admin: manage all pricing/catalog entries via Django Admin.

## 2026-02-19 — Ops Settings UI (Pricing + SiteConfig)

- Added an **in-app Ops Settings** hub (no Django admin required): `/ops/settings/`.
- Added **Billing & Plans** settings page: `/ops/settings/billing/`
  - Edit plan catalog prices, included seats, trial days, and Stripe Price IDs.
  - Edit extra seat add-on pricing and Stripe Price IDs.
- Billing & Plans: replaced the disabled Tier dropdown with a read-only badge (disabled selects looked broken and don’t submit values).
- Added **Site settings** page: `/ops/settings/site/`
