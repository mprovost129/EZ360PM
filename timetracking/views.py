...
def timer_panel(request):
    ...
    timer_state, _ = TimerState.objects.get_or_create(company=company, employee=employee)
    form = TimerStartForm(company=company, initial={"project": timer_state.project_id, "client": timer_state.client_id, "note": timer_state.note})
...
def timer_start(request):
    ...
    log_event(
        ...
        payload={"project_id": str(timer_state.project_id or ""), "client_id": str(timer_state.client_id or "")},
        request=request,
    )
...
def timer_stop(request):
    ...
    started = timer_state.started_at
    ended = timezone.now()
