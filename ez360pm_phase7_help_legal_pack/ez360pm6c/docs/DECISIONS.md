...
## 2026-02-13 — Phase 6D: Direct-to-S3 uploads must be presigned by the server
Decision:
- Large uploads (receipts, project files) should not hit the web dyno.
- The app generates short-lived **presigned POST** upload policies server-side and only after enforcing:
  - active company context
  - role rules
  - project access rules (staff must be assigned)

Rationale:
- Keeps tenant isolation and authorization correct.
- Improves reliability and latency for large uploads.
- Avoids dyno memory/timeout constraints.


## 2026-02-13 — Phase 6D: Storage Ops status + smoke tests
Decision:
- Add an Ops → Storage page that summarizes storage posture (S3 enabled, bucket names/regions, presign expiry) and records results of a simple smoke test.
- Smoke test should be non-destructive: write a small temp object to the private bucket via presigned POST, then immediately fetch via presigned GET and delete (if IAM permits), recording success/failure.

Rationale:
- Gives a fast, repeatable way to verify storage in staging/production.
- Prevents “S3 looks configured but isn’t actually working” deploy surprises.

## Storage posture: no ACLs + policy-based access (S3)
- Decision: Prefer **ACL-free** S3 posture (Object Ownership / bucket policy) rather than per-object ACLs.
- Implementation:
  - `AWS_DEFAULT_ACL=None`
  - Storage backends use `default_acl=None`
  - Private media is always served via signed URLs (querystring auth).
- Rationale: Simplifies security posture and aligns with modern AWS recommendations; avoids accidental public ACLs.

## Storage hardening evidence lives in Ops (Launch Gate)
- Decision: Treat S3 console-only settings (Block Public Access, CORS, Lifecycle) as **manual posture checks** recorded in-app via Ops → Storage.
- Rationale: These settings can’t be reliably inferred from app runtime config alone without AWS API permissions; capturing evidence in Ops keeps the launch gate auditable.


## 2026-02-14 — Help Center and Legal are template-first for launch
Decision:
- Implement Help Center and Legal pages as static templates (not DB-editable) for the initial launch.

Rationale:
- Fast, low-risk, no migration overhead.
- Keeps launch posture stable and version-controlled.
- We can add DB-managed CMS-style help articles later (Phase 8+) once core hardening is complete.
