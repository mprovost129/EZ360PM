...
### Phase 6B — Backup automation command (DONE)
...
## 2026-02-13 — Completed: Phase 6D.1–6D.2 (Direct uploads + project file downloads)
- Direct-to-S3 uploads implemented for:
  - Expense receipts
  - Project files
- Project file downloads now have a canonical `/download/` route (alias to the existing open handler).


### Completed: Phase 6D.3–6D.4 (S3 posture + staging verification)
- Enforced/validated S3 hardening defaults in settings:
  - `AWS_S3_SIGNATURE_VERSION=s3v4`
  - `AWS_DEFAULT_ACL=None` (preferred; no ACLs)
- Added Ops → Storage page:
  - Shows S3 enabled + buckets/locations/expiry
  - Run smoke tests (tiny upload + signed URL generation) and records last status per test
- Added Launch Checks coverage for S3 posture (signature, ACL, bucket set, expiry ranges)
- Added staging verification checklist directly on Ops → Storage page

### Completed: Phase 6D.5 (AWS console posture + lifecycle rules + documentation)
- Added Ops → Storage “Production readiness” checklist (manual posture checks) backed by Launch Gate items:
  - Block Public Access (private bucket)
  - CORS configured (presigned POST)
  - Lifecycle rules configured
  - E2E receipts + E2E project files verified
- Added copy/paste snippets on Ops → Storage for AWS CORS + lifecycle guidance.

### Next: Phase 6E (Production deployment pass)
- [ ] Configure Render/production env vars for S3 + presign expiry.
- [ ] Apply AWS console settings (Block Public Access, CORS, lifecycle) and mark the Ops checklist complete.
- [ ] Run real E2E UI tests in staging/production and record evidence in Ops.


## 2026-02-14 — Completed: Phase 7 (Help Center + Legal + Dead-End Sweep)

- In-app Help Center added (public + authenticated shells):
  - `/help/` home + module guides (getting started, roles/permissions, time, invoices/payments, accounting, storage, billing, ops, faq)
- Legal pages added + linked in footers:
  - `/legal/terms/`, `/legal/privacy/`, `/legal/cookies/`, `/legal/acceptable-use/`, `/legal/security/`, `/legal/refund-policy/`
- Dead-end fixes:
  - 404/500 dashboard links now point to `core:app_dashboard`
  - Getting Started numbering link now points to `documents:document_settings`
  - Time list “Add time entry” now points to `timetracking:entry_create`
- Repo documentation:
  - `docs/USER_MANUAL_v1.pdf` embedded
  - `docs/USER_MANUAL_v2.pdf` addendum embedded


## Next: Phase 8 (Pre-launch hardening recommendations)

- [ ] Add Audit Log viewer UI (filter/search/export)
- [ ] Confirm/implement soft-delete strategy for core records (clients/projects/documents/expenses/files)
- [ ] Transactional emails: invoice sent, payment recorded, employee invited
- [ ] Stripe webhook idempotency review + event storage hardening
- [ ] Data export (company full export + module CSVs)
- [ ] Production security finalization (DEBUG off, cookies, CSRF, ALLOWED_HOSTS, admin hardening)
- [ ] Error monitoring (Sentry) + structured logging verification
