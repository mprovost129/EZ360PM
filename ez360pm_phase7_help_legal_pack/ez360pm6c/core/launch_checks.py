...
def run_launch_checks() -> List[Dict[str, Any]]:
    ...
    out.append(
        _result(
            check_id="use_s3",
            title="S3 media storage enabled",
            ok=bool(use_s3),
            level="info",
            message="USE_S3=1" if use_s3 else "USE_S3=0 (local media)",
            hint="Enable USE_S3=1 in production to keep media off the web dyno.",
        )
    )

    if use_s3:
        sigver = getattr(settings, "AWS_S3_SIGNATURE_VERSION", "") or ""
        out.append(
            _result(
                check_id="s3_signature_v4",
                title="S3 signature version is v4",
                ok=(sigver == "s3v4"),
                level="error",
                message=f"AWS_S3_SIGNATURE_VERSION={sigver}",
                hint="Set AWS_S3_SIGNATURE_VERSION=s3v4 (required for most modern AWS regions).",
            )
        )
        ...
        out.append(
            _result(
                check_id="s3_default_acl_none",
                title="AWS_DEFAULT_ACL is None",
                ok=(default_acl in (None, "", "None")),
                level="warn",
                message=f"AWS_DEFAULT_ACL={default_acl}",
                hint="Prefer AWS_DEFAULT_ACL=None and enforce access via bucket policy/object ownership.",
            )
        )
        ...
        out.append(
            _result(
                check_id="s3_private_bucket_set",
                title="Private media bucket configured",
                ok=bool(private_bucket),
                level="error",
                message=f"S3_PRIVATE_MEDIA_BUCKET={private_bucket}",
                hint="Set S3_PRIVATE_MEDIA_BUCKET (or AWS_STORAGE_BUCKET_NAME) for private uploads/downloads.",
            )
        )

        post_exp = int(getattr(settings, "S3_PRESIGN_POST_EXPIRE_SECONDS", 300))
        out.append(
            _result(
                check_id="s3_presign_post_expiry",
                title="Presigned POST expiry is reasonable",
                ok=(60 <= post_exp <= 900),
                level="warn",
                message=f"S3_PRESIGN_POST_EXPIRE_SECONDS={post_exp}",
                hint="Recommended 300s (5 minutes).",
            )
        )

        dl_exp = int(getattr(settings, "S3_PRIVATE_MEDIA_EXPIRE_SECONDS", 600))
        out.append(
            _result(
                check_id="s3_private_url_expiry",
                title="Private signed URL expiry is reasonable",
                ok=(60 <= dl_exp <= 3600),
                level="warn",
                message=f"S3_PRIVATE_MEDIA_EXPIRE_SECONDS={dl_exp}",
                hint="Recommended 600s (10 minutes) for private media links.",
            )
        )
