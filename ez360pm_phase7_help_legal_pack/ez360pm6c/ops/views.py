...
from .models import OpsAlertEvent, OpsAlertSource, OpsAlertLevel, LaunchGateItem, BackupRun, BackupRestoreTest, BackupRunStatus, RestoreTestOutcome, ReleaseNote, StorageSmokeTest, StorageSmokeKind
...
@login_required
@user_passes_test(_is_staff)
def ops_storage(request: HttpRequest) -> HttpResponse:
    """Ops → Storage: summarize S3 posture and show smoke test results."""
    ...
    ctx = {
        ...
        "aws": {
            ...
            "signature_version": getattr(settings, "AWS_S3_SIGNATURE_VERSION", "") or "",
            "default_acl": getattr(settings, "AWS_DEFAULT_ACL", None),
        },
        "s3": {
            "public_bucket": getattr(settings, "S3_PUBLIC_MEDIA_BUCKET", "") or getattr(settings, "AWS_STORAGE_BUCKET_NAME", ""),
            "private_bucket": getattr(settings, "S3_PRIVATE_MEDIA_BUCKET", "") or getattr(settings, "AWS_STORAGE_BUCKET_NAME", ""),
            "public_location": getattr(settings, "S3_PUBLIC_MEDIA_LOCATION", "public-media"),
            "private_location": getattr(settings, "S3_PRIVATE_MEDIA_LOCATION", "private-media"),
            "private_url_expire_seconds": int(getattr(settings, "S3_PRIVATE_MEDIA_EXPIRE_SECONDS", 600)),
            "presign_post_expire_seconds": int(getattr(settings, "S3_PRESIGN_POST_EXPIRE_SECONDS", 300)),
        },
    }

    latest = {}
    for kind, _label in StorageSmokeKind.choices:
        latest[kind] = StorageSmokeTest.objects.filter(kind=kind).order_by("-created_at").first()
    ctx["latest_tests"] = latest

    # Manual posture checks (AWS Console / environment validation)
    posture_defaults = {
        "s3_block_public_access_private_bucket": {
            "title": "Private bucket: Block Public Access enabled",
            "description": "In AWS S3 console, ensure Block Public Access is ON for the private bucket.",
        },
        "s3_cors_configured_for_presigned_post": {
            "title": "CORS configured for presigned POST uploads",
            "description": "Confirm CORS rules allow browser direct uploads for your app origin(s).",
        },
        "s3_lifecycle_rules_configured": {
            "title": "Lifecycle rules configured",
            "description": "Add lifecycle rules for incomplete multipart uploads and optional cleanup of ops/smoke objects.",
        },
        "s3_e2e_receipt_upload_download": {
            "title": "E2E: Receipt upload + download verified",
            "description": "Upload an expense receipt via UI (direct-to-S3) and download it via signed URL.",
        },
        "s3_e2e_project_file_upload_download": {
            "title": "E2E: Project file upload + download verified",
            "description": "Upload a project file via UI (direct-to-S3) and download it via signed URL.",
        },
    }
    posture_items = []
    for key, meta in posture_defaults.items():
        item, _ = LaunchGateItem.objects.get_or_create(
            key=key,
            defaults={
                "title": meta["title"],
    ...
    ctx["posture_items"] = posture_items

    return render(request, "ops/storage.html", ctx)


@login_required
@user_passes_test(_is_staff)
@require_POST
def ops_storage_posture_toggle(request: HttpRequest) -> HttpResponse:
    """Toggle manual posture checks for S3 production readiness."""
    key = (request.POST.get("key") or "").strip()
    if not key:
        return redirect("ops:storage")

    item = LaunchGateItem.objects.filter(key=key).first()
    if not item:
        return redirect("ops:storage")

    item.is_complete = not item.is_complete
    if item.is_complete:
        item.completed_by = request.user.email or ""
    ...
    item.save(update_fields=["is_complete", "completed_by", "completed_at", "notes", "updated_at"])
    return redirect("ops:storage")


def _run_storage_smoke_test(*, kind: str) -> StorageSmokeTest:
    storage = PrivateMediaStorage()
    now = timezone.now().strftime("%Y%m%d%H%M%S")
    object_name = f"ops/smoke/{kind}/{now}_{uuid.uuid4().hex}.txt"

    # Upload: write tiny content
    try:
        saved_name = storage.save(object_name, ContentFile(f"smoke:{kind}:{now}".encode("utf-8")))
        msg = "Uploaded tiny object"
        ok = True
    except Exception as e:
        return StorageSmokeTest.objects.create(
            kind=kind,
            ok=False,
    ...
    # Download test: generate presigned URL (django-storages does this when querystring_auth=True)
    presigned_url = ""
    try:
        presigned_url = storage.url(saved_name)
    except Exception as e:
        return StorageSmokeTest.objects.create(
            kind=kind,
            ok=False,
            message="URL signing failed",
            object_name=saved_name,
            details={"error": str(e)},
        )

    details = {"presigned_url_generated": True}
    # Optional HEAD check (best-effort; safe to skip)
    if getattr(settings, "OPS_STORAGE_SMOKE_HTTP", False):
        try:
            import requests  # type: ignore

            r = requests.head(presigned_url, timeout=10)
            details["head_status_code"] = r.status_code
            if r.status_code >= 400:
                ok = False
                msg = f"HEAD failed ({r.status_code})"
        except Exception as e:
            ok = False
            msg = "HEAD request failed"
            details["head_error"] = str(e)
...
@user_passes_test(_is_staff)
@require_POST
def ops_storage_smoke_run(request: HttpRequest) -> HttpResponse:
    """Run storage smoke tests for receipt/project workflows."""
    kinds = [
        StorageSmokeKind.RECEIPT_UPLOAD,
        StorageSmokeKind.RECEIPT_DOWNLOAD,
        StorageSmokeKind.PROJECT_UPLOAD,
        StorageSmokeKind.PROJECT_DOWNLOAD,
    ]

    results = []
    for k in kinds:
        # We treat upload+download as the same primitive test (write + presign URL).
        results.append(_run_storage_smoke_test(kind=k))

    ok_count = sum(1 for r in results if r.ok)
    if ok_count == len(results):
        messages.success(request, "Storage smoke tests passed.")
    else:
        messages.error(request, f"Storage smoke tests completed with failures ({ok_count}/{len(results)} passed).")

    # Create/refresh a launch gate item to make Phase 6D.4 actionable
    try:
        item, _ = LaunchGateItem.objects.get_or_create(
            key="storage_smoke_tests",
            defaults={
                "title": "Storage smoke tests (S3 uploads/downloads)",
                "description": "Verify direct-to-S3 uploads and signed downloads for receipts and project files.",
            },
        )
        item.notes = f"Last run: {timezone.now().isoformat()}Z — {ok_count}/{len(results)} passed."
        item.is_complete = (ok_count == len(results))
        item.completed_by = (request.user.email or "") if item.is_complete else ""
        item.completed_at = timezone.now() if item.is_complete else None
        item.save(update_fields=["notes", "is_complete", "completed_by", "completed_at", "updated_at"])
    except Exception:
        pass
