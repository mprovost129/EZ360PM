...
class StorageSmokeKind(models.TextChoices):
    RECEIPT_UPLOAD = "receipt_upload", "Receipt upload"
    RECEIPT_DOWNLOAD = "receipt_download", "Receipt download"
    PROJECT_UPLOAD = "project_upload", "Project file upload"
    PROJECT_DOWNLOAD = "project_download", "Project file download"


class StorageSmokeTest(models.Model):
    """Records results of storage upload/download smoke tests.

    These tests are intended to be safe to run in staging/production:
    - Upload test: writes a tiny object to the private media bucket/prefix.
    - Download test: generates a presigned URL for the object (and optionally HEADs it).
    """

    created_at = models.DateTimeField(default=timezone.now, db_index=True)
    kind = models.CharField(max_length=32, choices=StorageSmokeKind.choices, db_index=True)

    ok = models.BooleanField(default=False, db_index=True)
    message = models.CharField(max_length=240, blank=True, default="")
    object_name = models.CharField(max_length=512, blank=True, default="")
    details = models.JSONField(default=dict, blank=True)
    ...
    def __str__(self) -> str:
        return f"{self.kind} ({'ok' if self.ok else 'fail'})"
