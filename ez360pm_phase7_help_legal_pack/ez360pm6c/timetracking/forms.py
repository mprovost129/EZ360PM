...
class TimerStartForm(forms.Form):
    ...
    client = forms.ModelChoiceField(queryset=Client.objects.none(), required=False, widget=forms.Select(attrs={"class": "form-select"}))
    ...
    def __init__(self, *args, company=None, **kwargs):
        ...
        if company is not None:
            ...
            self.fields["client"] = forms.ModelChoiceField(
                queryset=Client.objects.filter(company=company, deleted_at__isnull=True).order_by("company_name", "last_name", "first_name"),
                required=False,
                widget=forms.Select(attrs={"class": "form-select"})
            )
