...
import stripe
from stripe import SignatureVerificationError, error as stripe_error
...
EXPECTED_STRIPE_LIVEMODE: Optional[bool] = getattr(settings, "STRIPE_LIVE_MODE", None)
# If STRIPE_LIVE_MODE is True/False, ignore the opposite mode’s webhooks.


# ---------- Helpers ----------
def _site_url(request: HttpRequest) -> str:
    """Prefer settings.SITE_URL, fall back to request base URL."""
...
def _idem_key(prefix: str, request: HttpRequest, *parts: Any) -> str:
    """
    Build a per-minute idempotency key. Ensures session_key exists.
    """
    if not request.session.session_key:
        request.session.save()
    minute_bucket = int(time.time() // 60)
    return f"{prefix}:{request.session.session_key}:{minute_bucket}:" + ":".join(map(str, parts))
...
def subscribe(request: HttpRequest, slug: str) -> HttpResponse:
    ...
    idem_key = _idem_key("sub", request, company.id, tier.slug) # type: ignore
...
def portal(request: HttpRequest) -> HttpResponse:
    ...
    idem_key = _idem_key("portal", request, company.id) # type: ignore
...
def webhook_stripe(request: HttpRequest) -> HttpResponse:
    ...
    # Optional live/test guard
    if EXPECTED_STRIPE_LIVEMODE is not None:
        evt_livemode = bool(event.get("livemode", False))
        if evt_livemode != EXPECTED_STRIPE_LIVEMODE:
            logger.info("Ignoring webhook in wrong mode (event.livemode=%s)", evt_livemode)
    ...
    # De-dup: if handled already, exit fast
    if WebhookLog.objects.filter(stripe_event_id=event_id, processed_ok=True).only("id").exists():
        return HttpResponse(status=200)

    # Create (or fetch) a log record for this event
    log, created = WebhookLog.objects.get_or_create(
        stripe_event_id=event_id,
        defaults={"type": type_ or "", "raw": event},
    )
    if not created:
        log.type = type_ or log.type
        log.raw = event
    ...
    try:
        ...
        logger.exception("Stripe webhook handler error for %s", type_)
...
def _handle_checkout_completed(session: dict[str, Any]) -> None:
    ...
    from company.models import Company  # local import to avoid cycles
    ...
    except stripe_error.StripeError: # type: ignore
        logger.exception("Stripe Subscription.retrieve failed for %s", subscription_id)
        return

    sub.status = s.status  # type: ignore[attr-defined]
    sub.current_period_end = timezone.datetime.fromtimestamp(  # type: ignore[assignment]
        s.current_period_end, tz=timezone.utc  # type: ignore[attr-defined]
    )
    sub.cancel_at_period_end = bool(getattr(s, "cancel_at_period_end", False))

    # Map current price -> tier (in case Checkout used a different price)
    try:
        items = (s.get("items") or {}).get("data", [])  # type: ignore[attr-defined]
        price_id = (items[0].get("price") or {}).get("id") if items else None
        if price_id:
            mapped = SubscriptionTier.objects.filter(stripe_price_id=price_id).first()
            if mapped:
                sub.tier = mapped
    except Exception:
        pass
...
def _handle_subscription_event(s: dict[str, Any]) -> None:
    ...
    # Map current price -> tier (user might have changed plan in Portal)
    try:
        items = (s.get("items") or {}).get("data", [])
        price_id = (items[0].get("price") or {}).get("id") if items else None
        if price_id:
            tier = SubscriptionTier.objects.filter(stripe_price_id=price_id).first()
            if tier and sub.tier_id != tier.id:  # type: ignore[attr-defined]
                sub.tier = tier
    except Exception:
        pass
...
def _handle_invoice_payment(obj: dict[str, Any]) -> dict[str, Any]:
    ...
    # Fallback: fetch PaymentIntent's metadata if this is a Checkout Session without metadata
    if not inv and obj.get("object") == "checkout.session":
        pi_id = obj.get("payment_intent")
        if pi_id:
            try:
                pi = stripe.PaymentIntent.retrieve(pi_id)
                inv = _find_invoice_from_metadata(pi.get("metadata") or {})
            except Exception:
                inv = None
    ...
    if found_inv:
        ...
        try:
            notify_company(
                ...
                url=reverse("invoices:invoice_detail", args=[found_inv.pk]),
