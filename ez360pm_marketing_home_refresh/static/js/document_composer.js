  ...
  function setup() {
    ...
    function refreshSortOrders() {
      ...
      qsa('tr[data-ez-line="1"]', tbody).forEach(function (row) {
        var so = qs('input[name$="-sort_order"]', row);
        if (so) {
          so.value = String(i);
        }
        i += 1;
    ...
    tbody.addEventListener('click', function (e) {
      var btn = e.target.closest('[data-ez-move]');
      if (!btn) return;
      e.preventDefault();
      var dir = btn.getAttribute('data-ez-move');
      var row = btn.closest('tr[data-ez-line="1"]');
      moveRow(row, dir);
      computeTotals();
    });
    ...
    tbody.addEventListener('dragstart', function (e) {
      var row = e.target.closest('tr[data-ez-line="1"]');
      if (!row) return;
      dragSrc = row;
    ...
    tbody.addEventListener('dragend', function (e) {
      var row = e.target.closest('tr[data-ez-line="1"]');
      if (row) row.classList.remove('ez-dragging');
      dragSrc = null;
    });
    tbody.addEventListener('dragover', function (e) {
      ...
      var over = e.target.closest('tr[data-ez-line="1"]');
      ...
      if (before) {
        ...
        tbody.insertBefore(dragSrc, over.nextElementSibling);
      }
    });
