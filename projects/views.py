...
def project_files(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Either traditional multipart upload OR S3 direct upload (file_s3_key).
            upload = request.FILES.get('file')
            file_s3_key = (form.cleaned_data.get("file_s3_key") or "").strip()
            content_bytes = upload.read() if upload else b''

            if file_s3_key and not upload:
                # Validate key prefix to prevent cross-company/project access.
                priv_loc = (getattr(settings, "S3_PRIVATE_MEDIA_LOCATION", "private-media") or "private-media").strip("/")
                expected_prefix = f"{priv_loc}/projects/{company.id}/{project.id}/"
                if not file_s3_key.startswith(expected_prefix):
                    messages.error(request, "Invalid upload key.")
                    return redirect("projects:project_files", pk=project.id)
                pf.file.name = file_s3_key
    ...
    return render(
        ...
        "projects/project_files.html",
        {
            ...
            "is_manager": bool(employee and employee.role in (EmployeeRole.MANAGER, EmployeeRole.ADMIN, EmployeeRole.OWNER)),
        },
    )
