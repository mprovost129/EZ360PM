...
class LineItemForm(forms.ModelForm):
    unit_price_dollars = forms.DecimalField(required=False, min_value=Decimal("0.00"), decimal_places=2, max_digits=12)
    tax_dollars = forms.DecimalField(required=False, min_value=Decimal("0.00"), decimal_places=2, max_digits=12)

    class Meta:
        model = DocumentLineItem
        fields = ["sort_order", "catalog_item", "name", "description", "qty", "unit_price_dollars", "is_taxable", "tax_dollars"]
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            self.fields["unit_price_dollars"].initial = (Decimal(int(self.instance.unit_price_cents or 0)) / 100).quantize(Decimal("0.01"))
            self.fields["tax_dollars"].initial = (Decimal(int(self.instance.tax_cents or 0)) / 100).quantize(Decimal("0.01"))

    def clean(self):
        cleaned = super().clean()
        qty = cleaned.get("qty") or Decimal("0")
        unit_price = cleaned.get("unit_price_dollars") or Decimal("0")
        tax = cleaned.get("tax_dollars") or Decimal("0")
        unit_cents = int((unit_price * 100).quantize(Decimal("1")))
        tax_cents = int((tax * 100).quantize(Decimal("1")))
