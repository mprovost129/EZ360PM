...
class TimerStartForm(forms.Form):
    ...
    save_service_to_catalog = forms.BooleanField(
        required=False,
        label="Save as catalog service",
        widget=forms.CheckboxInput(attrs={"class": "form-check-input"}),
    )
    note = forms.CharField(required=False, widget=forms.Textarea(attrs={"class": "form-control", "rows": 2}))

    def __init__(self, *args, company=None, can_manage_catalog: bool = False, **kwargs):
        super().__init__(*args, **kwargs)
        if not can_manage_catalog:
            self.fields.pop("save_service_to_catalog", None)
    ...
    def clean(self):
        ...
        if cleaned.get("service_catalog_item"):
            ...
            cleaned["save_service_to_catalog"] = False
        return cleaned
