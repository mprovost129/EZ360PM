...
def get_plan_catalog(plan_code: str) -> PlanCatalog | None:
    try:
        return PlanCatalog.objects.filter(code=plan_code, is_active=True).first()
    except Exception:
        # During early migrations or if table missing, fall back to constants.
        return None


def get_seat_addon_config() -> SeatAddonConfig | None:
    try:
        return SeatAddonConfig.objects.filter(id=1).first()
    except Exception:
        return None


def trial_days_for_plan(plan_code: str) -> int:
    plan = get_plan_catalog(plan_code)
    if plan and getattr(plan, "trial_days", None):
        return int(plan.trial_days)
    return int(getattr(settings, "EZ360PM_TRIAL_DAYS", 14) or 14)


def included_seats_for_plan_code(plan_code: str) -> int:
    plan = get_plan_catalog(plan_code)
    if plan and getattr(plan, "included_seats", None) is not None:
        return int(plan.included_seats)
    return int(INCLUDED_SEATS.get(plan_code, 1))


def _trial_days(subscription: CompanySubscription | None = None) -> int:
    plan_code = getattr(subscription, 'plan', PlanCode.STARTER) if subscription else PlanCode.STARTER
    return trial_days_for_plan(str(plan_code))
...
def ensure_trial_initialized(subscription: CompanySubscription) -> CompanySubscription:
    ...
    if not subscription.trial_ends_at:
        subscription.trial_ends_at = now + timedelta(days=_trial_days(subscription))
        changed = True
    if changed:
...
def included_seats_for(plan: str) -> int:
    return included_seats_for_plan_code(plan)
