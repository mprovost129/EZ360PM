...
class Command(BaseCommand):
    ...
    def _render_pdf_or_none(self, inv: Invoice) -> Optional[bytes]:
        """
        Try to render PDF. If rendering tools aren't available, skip attachment gracefully.
        Preference order:
          1) invoices.pdf.render_invoice_pdf
          2) core.pdf._render_pdf_from_html
          3) core.views._render_pdf_from_html
        """
        try:
            try:
                # Prefer a dedicated invoices PDF helper, if provided
                from invoices.pdf import render_invoice_pdf  # type: ignore
                return render_invoice_pdf(inv)
            except Exception:
                # Fallback: render HTML then convert
    ...
    def _send_one(self, inv: Invoice, days: int, *, dry_run: bool) -> bool:
        ...
        # Build a public URL for the invoice
        try:
