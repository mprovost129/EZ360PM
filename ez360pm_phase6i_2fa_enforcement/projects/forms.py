...
class ProjectForm(forms.ModelForm):
    ...
    hourly_rate_dollars = forms.DecimalField(required=False, min_value=Decimal("0.00"), decimal_places=2, max_digits=12)
    flat_fee_dollars = forms.DecimalField(required=False, min_value=Decimal("0.00"), decimal_places=2, max_digits=12)

    class Meta:
        model = Project
        fields = [
            'client','assigned_to','project_number','name','description',
            'date_received','due_date','billing_type','hourly_rate_dollars','flat_fee_dollars',
        ]
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Initialize dollar helpers from cents
        if self.instance and self.instance.pk:
            self.fields['hourly_rate_dollars'].initial = (Decimal(int(self.instance.hourly_rate_cents or 0)) / Decimal('100')).quantize(Decimal('0.01'))
            self.fields['flat_fee_dollars'].initial = (Decimal(int(self.instance.flat_fee_cents or 0)) / Decimal('100')).quantize(Decimal('0.01'))

        # Ensure currency fields are numeric with cents precision
        self.fields['hourly_rate_dollars'].widget.attrs.setdefault('step', '0.01')
        self.fields['flat_fee_dollars'].widget.attrs.setdefault('step', '0.01')
    ...
    def clean(self):
        ...
        hourly = cleaned.get('hourly_rate_dollars') or Decimal('0')
        flat = cleaned.get('flat_fee_dollars') or Decimal('0')
        cleaned['hourly_rate_cents'] = int((hourly * 100).quantize(Decimal('1')))
        cleaned['flat_fee_cents'] = int((flat * 100).quantize(Decimal('1')))
        return cleaned
