...
class NumberingScheme(SyncModel):
    ...
    # Optional reset behavior for pattern-based numbering.
    # If enabled, the SEQ counter resets when the period changes.
    invoice_reset = models.CharField(max_length=10, choices=ResetPolicy.choices, default=ResetPolicy.NONE)
    estimate_reset = models.CharField(max_length=10, choices=ResetPolicy.choices, default=ResetPolicy.NONE)
    proposal_reset = models.CharField(max_length=10, choices=ResetPolicy.choices, default=ResetPolicy.NONE)

    invoice_seq_period = models.CharField(max_length=8, blank=True, default="")
    estimate_seq_period = models.CharField(max_length=8, blank=True, default="")
    proposal_seq_period = models.CharField(max_length=8, blank=True, default="")
...
class Document(SyncModel):
    ...
    use_project_numbering = models.BooleanField(
        default=False,
    ...
    def _enforce_invoice_immutability(self, previous: "Document") -> None:
        ...
        if locked_before:
            immutable_fields = [
                ("number", previous.number, self.number),
                ("use_project_numbering", previous.use_project_numbering, self.use_project_numbering),
...
class ProjectDocSequence(models.Model):
    """Per-project sequence counter for PROJECTNUMBER-1, PROJECTNUMBER-2… style numbering."""

    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name="project_doc_sequences")
    project = models.ForeignKey(Project, on_delete=models.CASCADE, related_name="doc_sequences")
    doc_type = models.CharField(max_length=20, choices=DocumentType.choices)
    next_seq = models.BigIntegerField(default=1)

    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["company", "project", "doc_type"], name="uniq_company_project_doctype_seq"),
        ]

    def __str__(self) -> str:
        return f"{self.company_id}:{self.project_id}:{self.doc_type}:{self.next_seq}"
